...
unsigned long lastPrintEverything = 0;
long printEverythingTime = 100; // print every x milliseconds
...
void loop() {
  ...
  // shutdown everything, hard stop motors when dpad_x = 1 (right cross on dpad pressed)
   if (dpad_x == 1) {
      hardStop();
    } 
  ...
  // print desired speed, actual speed, error, integral error, abs(PWM) applied to wheel
    if (((millis() - lastPrintEverything) >= printEverythingTime) && printEverything) {
      for (i=0; i<4; i++) {
        Serial.print(desired_wheel_speed[i]);
        Serial.print(",");
        Serial.print(actual_wheel_speed[i]);
        Serial.print(",");
        Serial.print(error[i]);
        Serial.print(",");
        Serial.print(integral_error[i]);
        Serial.print(",");
        Serial.println(255 * Voltage[i] / BATTERY_VOLTAGE);

      }
      lastPrintEverything = millis();
    }
