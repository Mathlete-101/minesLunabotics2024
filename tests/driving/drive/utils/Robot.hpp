...
// motor drivers
CytronMD right_F_Mtr(PWM_DIR, RIGHT_F_PWM, RIGHT_F_DIR);
CytronMD left_F_Mtr(PWM_DIR, LEFT_F_PWM, LEFT_F_DIR);
CytronMD right_B_Mtr(PWM_DIR, RIGHT_B_PWM, RIGHT_B_DIR);
CytronMD left_B_Mtr(PWM_DIR, LEFT_B_PWM, LEFT_B_DIR);
...
Robot::Robot() {
    ...
    right_F_Enc = new Encoder(ENCR_A_FR, ENCR_B_FR);
    left_F_Enc = new Encoder(ENCR_A_FL, ENCR_B_FL);
    right_B_Enc = new Encoder(ENCR_A_BR, ENCR_B_BR);
    left_B_Enc = new Encoder(ENCR_A_BL, ENCR_B_BL);

    this->tracker = new Tracker(right_F_Enc, left_F_Enc, right_B_Enc, left_B_Enc); // update tracker
...
Robot::~Robot() {
    ...
    delete tracker, right_F_Enc, left_F_Enc, right_B_Enc, left_B_Enc;
...
void Robot::turnInPlace(double desAngleRad) {
    ...
    while ((abs(error) > delta || abs(phiVelAct) > 0.05 || abs(rhoVelAct) > 0.05) && (settledTimeCounter < 1000)) {
        ...
        // drive motors
        right_F_Mtr.setSpeed(-volts2pwm(voltages.getVright()));
        left_F_Mtr.setSpeed(-volts2pwm(voltages.getVleft()));
        right_B_Mtr.setSpeed(-volts2pwm(voltages.getVright()));
        left_B_Mtr.setSpeed(-volts2pwm(voltages.getVleft()));
...
void Robot::stop() {
    right_F_Mtr.setSpeed(0);
    right_F_Mtr.setSpeed(0);
    left_B_Mtr.setSpeed(0);
    left_B_Mtr.setSpeed(0);
}
